...
const Input = ({ type, name, onChange, onClick, value, options, label, valueKey = 'name', children, loading, placeholder, ...props }) => {
  ...
  return <div {...props}>
    ...
    {type === 'select' && !!options && <div>
      <span className="input button dropdown" name={name}>
        <div>{Object.entries(options.filter(item => item.id === value)[0] || {[valueKey]: placeholder || 'Select'}).map(([entryKey, value]) => entryKey === valueKey && value)}</div>
    ...
    {!['submit', 'select'].includes(type) && <input className='input' placeholder={placeholder} type={type} name={name} onChange={(e) => handleOnChange(e, e.target.value)} value={value} />}
  </div>
}
